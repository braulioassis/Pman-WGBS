...
cor_results <- read.csv("w.spearman.CPM.csv")
cor_results <- na.omit(cor_results)
cor_results$bin <- cut(cor_results$rho, breaks = seq(floor(min(cor_results$rho)), ceiling(max(cor_results$rho)), by = 0.1), include.lowest = T)
...
sig_results <- filter(cor_results, abs(rho) >= 0.3, fdr < 0.05)
...
cor_results <- read.csv("lz.spearman.CPM.csv")
cor_results <- na.omit(cor_results)
cor_results$bin <- cut(cor_results$rho, breaks = seq(floor(min(cor_results$rho)), ceiling(max(cor_results$rho)), by = 0.1), include.lowest = T)
cor_counts <- as.data.frame(table(cor_results$bin))
colnames(cor_counts) <- c("bin", "count")
...
cor_results <- read.csv("jz.spearman.CPM.csv")
cor_results <- na.omit(cor_results)
cor_results$bin <- cut(cor_results$rho, breaks = seq(floor(min(cor_results$rho)), ceiling(max(cor_results$rho)), by = 0.1), include.lowest = T)
cor_counts <- as.data.frame(table(cor_results$bin))
colnames(cor_counts) <- c("bin", "count")
